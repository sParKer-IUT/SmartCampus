...
namespace SmartCampus
{
    public partial class ShowAttendance : UserControl
    {
        ...
        public ShowAttendance(string t)
        {
            InitializeComponent();
            type = t;
            if (type.Equals("in"))
        ...
        private void ShowAttendance_Load(object sender, EventArgs e)
        {
            ...
            string q = null;
            if(type.Equals("in")) q = "SELECT a.id,a.name,b.time,a.messaging_number FROM student_info a, dailyattendance b WHERE a.class ='" + AttendanceShow.sClass + "' and a.card_id=b.id and b.time like '" + tempTime + "';";
            else if (type.Equals("out")) q = "SELECT a.id,a.name,b.time,a.messaging_number FROM student_info a, dailycheckout b WHERE a.class ='" + AttendanceShow.sClass + "' and a.card_id=b.id and b.time like '" + tempTime + "';";
            sc = new MySqlCommand(q, connection);
            reader = sc.ExecuteReader();
            while(reader.Read())
            {
                Data.Add(new data() { Name = reader[1].ToString(), ID = reader[0].ToString(), Time = reader[2].ToString().Substring(reader[2].ToString().Length - 10), msg = reader[3].ToString()});
            }
        ...
        void updateGrid()
        {
            ...
            if (type.Equals("in"))
            {
                ...
                q = "SELECT a.id,a.name,b.time,a.messaging_number FROM student_info a, dailyattendance b WHERE a.class ='" + AttendanceShow.sClass + "' and a.card_id=b.id and b.time like '" + tempTime + "';";
            }
            else if (type.Equals("out"))
            {
                printSubtitle = "Checkout List"; 
                q = "SELECT a.id,a.name,b.time,a.messaging_number FROM student_info a, dailycheckout b WHERE a.class ='" + AttendanceShow.sClass + "' and a.card_id=b.id and b.time like '" + tempTime + "';";
            }
            string query = q;
            ...
            if(rbAbsent.Checked)
            {
                ...
                query = "SELECT id, name, messaging_number FROM `student_info` WHERE class = '" + AttendanceShow.sClass + "' and card_id NOT IN (SELECT id FROM `dailyattendance` WHERE class = '" + AttendanceShow.sClass + "' and time like '" + tempTime + "');";
            }

            printSubtitle += " of Students of class " + AttendanceShow.sClass + " in " + tempTime.ToString();
            ...
            while (reader.Read())
            {
                ...
                if (!rbAbsent.Checked)
                {
                    cintime = reader[2].ToString().Substring(reader[2].ToString().Length - 10);
                    Data.Add(new data() { Name = reader[1].ToString(), ID = reader[0].ToString(), Time = cintime, msg = reader[3].ToString() });
                }
                else
                {
                    cintime = "--";
                    Data.Add(new data() { Name = reader[1].ToString(), ID = reader[0].ToString(), Time = cintime, msg = reader[2].ToString() });
                }
        ...
        private void print_Click(object sender, EventArgs e)
        {
            ...
            printer.ColumnWidths.Add(dataGridView1.Columns[0].Name, 250);
            printer.ColumnWidths.Add(dataGridView1.Columns[1].Name, 150);
            printer.ColumnWidths.Add(dataGridView1.Columns[2].Name, 150);
            printer.ColumnWidths.Add(dataGridView1.Columns[3].Name, 150);
